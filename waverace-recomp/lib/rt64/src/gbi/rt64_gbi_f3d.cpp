...
namespace RT64 {
    // Debug counter for DL depth tracking
    static int g_dl_depth = 0;
    static int g_dl_debug_count = 0;
    namespace GBI_F3D {
        ...
        void endDl(State *state, DisplayList **dl) {
            DisplayList *returnAddr = state->popReturnAddress();
            if (g_dl_depth > 0) g_dl_depth--;
            *dl = returnAddr;
        }
