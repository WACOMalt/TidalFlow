...
RT64::UserConfiguration::Antialiasing compute_max_supported_aa(RenderSampleCounts bits) {
    if (bits & RenderSampleCount::Bits::COUNT_2) {
        if (bits & RenderSampleCount::Bits::COUNT_4) {
            if (bits & RenderSampleCount::Bits::COUNT_8) {
...
void zelda64::renderer::RT64Context::send_dl(const OSTask* task) {
    fprintf(stderr, "[RT64] send_dl: START - ucode=0x%08X ucode_data=0x%08X data_ptr=0x%08X\n",
            task->t.ucode, task->t.ucode_data, task->t.data_ptr);

    fprintf(stderr, "[RT64] send_dl: check_texture_pack_actions...\n");
    check_texture_pack_actions();

    fprintf(stderr, "[RT64] send_dl: rsp->reset()...\n");
    app->state->rsp->reset();

    fprintf(stderr, "[RT64] send_dl: loadUCodeGBI(ucode=0x%06X, ucode_data=0x%06X)...\n",
            task->t.ucode & 0x3FFFFFF, task->t.ucode_data & 0x3FFFFFF);
    app->interpreter->loadUCodeGBI(task->t.ucode & 0x3FFFFFF, task->t.ucode_data & 0x3FFFFFF, true);

    fprintf(stderr, "[RT64] send_dl: processDisplayLists(data_ptr=0x%06X)...\n",
            task->t.data_ptr & 0x3FFFFFF);
    app->processDisplayLists(app->core.RDRAM, task->t.data_ptr & 0x3FFFFFF, 0, true);

    fprintf(stderr, "[RT64] send_dl: DONE\n");
}
