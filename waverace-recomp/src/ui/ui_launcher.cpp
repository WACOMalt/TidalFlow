...
// Auto-start game for debugging (set to true to enable)
static constexpr bool AUTO_START_GAME = true;
...
public:
    ...
    void make_bindings(Rml::Context* context) override {
        ...
        // Auto-start game if enabled and ROM is valid
        if constexpr (AUTO_START_GAME) {
            if (mm_rom_valid) {
                // Spawn a thread that waits a bit then auto-starts
                std::thread([]() {
                    // Wait for UI to fully initialize
                    std::this_thread::sleep_for(std::chrono::milliseconds(500));
                    fprintf(stderr, "[AUTO-START] ROM is valid, auto-starting game...\n");
                    recomp::start_game(supported_games[0].game_id);
                    recompui::hide_all_contexts();
                }).detach();
            }
