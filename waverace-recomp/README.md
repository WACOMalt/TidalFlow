...
Wave Race 64: Recompiled is a project that uses [N64: Recompiled](https://github.com/Mr-Wiseguy/N64Recomp) to **statically recompile** Wave Race 64 into a native port. This project uses [RT64](https://github.com/rt64/rt64) as the rendering engine.

**Status**: Work in Progress - Main code recompiled (765 functions), overlay support in development.

### **This repository does not contain game assets. The original ROM is required.**

---

## AI-Assisted Development

This project is developed using **Claude Code with Claude Opus 4.5** as an experiment in AI-assisted reverse engineering. The AI assists with:

- ROM analysis and function boundary detection
- Symbol extraction and configuration generation
...
1. **Analysis Phase**: AI analyzes ROM data, identifies code segments, detects function boundaries
2. **Configuration Phase**: AI generates N64Recomp configuration files and symbol definitions
3. **Iteration Phase**: Build → Error → AI Analysis → Fix → Repeat
4. **Documentation Phase**: AI generates comprehensive documentation for reproducibility

### AI-Generated Scripts

| Script | Purpose |
|--------|---------|
| `generate_symbols.py` | Extract function symbols from decomp data |
| `auto_stub.py` | Automatically stub failing functions |
| `patch_stubs.py` | Patch stubs to call custom implementations |
| `find_bss_funcs.py` | Analyze BSS segment functions |

### Documentation

See `chris docs/` directory:

| Document | Description |
|----------|-------------|
| `PROJECT_STATUS.md` | Current status, timeline, known issues |
| `N64RECOMP_GUIDE.md` | How N64Recomp works, configuration |
| `BUILD_GUIDE.md` | General build instructions (any platform) |
| `MY_SETUP.md` | Chris's specific setup (Windows + WSL + Claude Code) |
| `AI_WORKFLOW.md` | AI-assisted development methodology |

**For LLMs**: Use `MY_SETUP.md` if working in Chris's environment, or `BUILD_GUIDE.md` for general setup.

### Why AI-Assisted?

Traditional N64 recompilation requires deep expertise in MIPS assembly, N64 hardware, and libultra. AI assistance:

- Accelerates pattern recognition in binary data
- Generates automation scripts quickly
- Cross-references similar implementations from reference projects
- Maintains comprehensive documentation
- Enables rapid iteration on complex problems
